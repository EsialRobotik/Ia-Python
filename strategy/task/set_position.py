...
class SetPosition(AbstractTask):

    def __init__(self, desc: str, position_x: int, position_y : int, angle_theta : float, mirror: Mirror = Mirror.MIRRORY):
        super().__init__(
            desc=desc,
    ...
    def execute(self, start_point: Position):
        self.end_point = Position(self.position_x, self.position_y, self.dist)
        return {
            "task": self.desc,
            "command": f"position#{self.position_x};{self.position_y};{self.dist}",
