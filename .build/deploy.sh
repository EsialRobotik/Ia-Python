...
then
    ...
    scp -r ia config requirements.txt $DEPLOY_SERVER_USER@$DEPLOY_SERVER_HOST:$DEPLOY_SERVER_DIR
else
    ssh -i "$DEPLOY_SERVER_IDENTITY_FILE_PATH" $DEPLOY_SERVER_USER@$DEPLOY_SERVER_HOST "mkdir -p $DEPLOY_SERVER_DIR"
    scp -i "$DEPLOY_SERVER_IDENTITY_FILE_PATH" -r ia config requirements.txt $DEPLOY_SERVER_USER@$DEPLOY_SERVER_HOST:$DEPLOY_SERVER_DIR
fi